...
public class LocalPdf {
	...
	public PdfPTable setTituloLogo ( Filtro criterio,Font mainFont) throws DocumentException{
		...
		try{
			...
			Paragraph paragraph = this.setTitlePdf("RELACIÓN DE LOCALES - "+ criterio.getNombre() + " \n"+ this.mesAnio(criterio), mainFont);
	...
	public PdfPTable setDataDeuda(List<LocalBean> listaLocal, Integer primeraVez) throws DocumentException{
		...
		if (primeraVez.compareTo(1) == 0){
			...
			headerCell = this.setHeaderPdf("ESTADO", tableHeader);
			table.addCell(headerCell);
		...
		for(LocalBean localBean:listaLocal ){
			...
			cellData = this.setDataPdf(localBean.getEstado(), tableBody, Element.ALIGN_LEFT, Element.ALIGN_CENTER);
			table.addCell(cellData);
