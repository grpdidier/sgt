...
public class MorosoLuzExcel {
	...
	public void reporteMorosoLuz(List<MorosoLuzBean> listaLuz, CriterioReporteBean criterio,ServletContext context,HttpServletRequest request, HttpServletResponse response, String nameFile) throws IOException{
		String[] columns = {"Nombre del Cliente", "Tienda", "Monto S/", "Fecha Vencimiento"};
		...
		Sheet sheet = workbook.createSheet("Luz");
		...
		cellTitulo.setCellValue("REPORTE DE TODOS LOS MOROSOS DE LUZ A LA FECHA");
	    ...
	    for(MorosoLuzBean moroso: listaLuz){
	    	...
	    	row.createCell(2).setCellValue(moroso.getMontoLuz().doubleValue());
