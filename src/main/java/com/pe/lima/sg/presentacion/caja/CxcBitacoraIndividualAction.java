...
public class CxcBitacoraIndividualAction extends BaseOperacionPresentacion<TblCxcBitacora> {
	...
	private String urlPaginado = "/cxis/paginado/"; 
	 ...
	 * @return
	 */
	@RequestMapping(value = "/cxis", method = RequestMethod.GET)
	 ...
	 * @return
	 */
	@RequestMapping(value = "/cxis/contrato/q", method = RequestMethod.POST)
	...
	private void listarContratos(Model model, TblContrato tblContrato,  PageableSG pageable, String url){
	 ...
	 * @return
	 */
	@RequestMapping(value = "/cxi/individual/{id}", method = RequestMethod.GET)
	public String traerRegistrosBitacora(@PathVariable Integer id, Model model, HttpServletRequest request) {
	...
	/*
	 * Obtiene el año de la tabla de parametros o por default
	 */
	public Integer mGetAnio(){
	 ...
	 * @return
	 */
	@RequestMapping(value = "/cxis/q", method = RequestMethod.POST)
	 ...
	 * @return
	 */
	@RequestMapping(value = "cxi/nuevo", method = RequestMethod.GET)
	public String crearBitacora(Model model, HttpServletRequest request) {
		...
		return "caja/cxi_individual/cxi_nuevo";
	}
	/**
	 * Se encarga de direccionar a la pantalla de creacion de una bitacora
	 * 
	 ...
	 * @return
	 */
	@RequestMapping(value = "cxi/regresar", method = RequestMethod.GET)
	public String regresarBitacora(Model model, HttpServletRequest request) {
		BitacoraBean filtro = null;
		List<TblCxcBitacora> entidades 		= null;
		try{
		...
		return "caja/cxi_individual/cxi_listado";
	}
	@Override
	 ...
	 * @return
	 */
	@RequestMapping(value = "cxi/nuevo/guardar", method = RequestMethod.POST)
		...
		try{
			...
			if (this.validarNegocio(model, entidad, request)){
				...
				if (exitoso){
					...
					path = "caja/cxi_individual/cxi_nuevo";
					this.cargarListaCuentasxCobrar(model, entidad);
					model.addAttribute("entidad", entidad);
				}
			}else{
	...
	public boolean procesarTipoCobro(Model model, BitacoraBean entidad, HttpServletRequest request){
		...
		try{
			...
			if (entidad.getAlquiler().equals(Constantes.TIPO_SI)){
				//Validamos que no se haya generado la carga masiva para ese año y mes y para el contrato
				intTotalRegistros =cxcBitacoraDao.countTipoCobroxContrato(entidad.getCodigoContrato(), entidad.getAnio(), new Integer(entidad.getMes()), Constantes.TIPO_COBRO_ALQUILER);
				if (intTotalRegistros !=null && intTotalRegistros >0){
					...
					listaContrato = contratoDao.listAllContratoActivosxFechaxContrato(entidad.getCodigoContrato(), UtilSGT.getDatetoString(UtilSGT.getFistDay(entidad.getMes(), entidad.getAnio())));
					if (listaContrato != null && listaContrato.size()>0){
						//Validamos la existencia del periodo para el contrato en la bitacora
						
						//Registramos la Bitacora del CxC del alquiler
						boolean exitoso = registrarBitacora(model, entidad, Constantes.TIPO_COBRO_ALQUILER, Constantes.SERIE_TIPO_OPERACION_INDIVIDUAL, request);
			...
			if (entidad.getArbitrio().equals(Constantes.TIPO_SI)){
				...
				resultado = false;
				bitacora = null;
				intTotalRegistros =cxcBitacoraDao.countTipoCobroxContrato(entidad.getCodigoContrato(), entidad.getAnio(), new Integer(entidad.getMes()), Constantes.TIPO_COBRO_ARBITRIO);
				if (intTotalRegistros >0){
					...
					listaArbitrio = arbitrioDao.listarAllActivosxFechaxContrato(entidad.getCodigoContrato(), UtilSGT.getDatetoString(fecha));
					if (listaArbitrio !=null){
						boolean exitoso = registrarBitacora(model, entidad, Constantes.TIPO_COBRO_ARBITRIO, Constantes.SERIE_TIPO_OPERACION_INDIVIDUAL, request);
						if (exitoso) {
							criterio = Specifications.where(conAnio(entidad.getAnio()))
									.and(conMes(new Integer(entidad.getMes())))
			...
			if (entidad.getServicio().equals(Constantes.TIPO_SI)){
				...
				resultado = false;
				bitacora = null;
				intTotalRegistros =cxcBitacoraDao.countTipoCobroxContrato(entidad.getCodigoContrato(), entidad.getAnio(), new Integer(entidad.getMes()), Constantes.TIPO_COBRO_SERVICIO);
				if (intTotalRegistros >0){
					...
					listaServicio = servicioDao.listarAllActivosxFechaxContrato(entidad.getCodigoContrato(), UtilSGT.getDatetoString(UtilSGT.getFistDay(entidad.getMes(), entidad.getAnio())));
					if (listaServicio != null && listaServicio.size()>0){
						//Registramos la Bitacora del CxC del alquiler
						boolean exitoso = registrarBitacora(model, entidad, Constantes.TIPO_COBRO_SERVICIO, Constantes.SERIE_TIPO_OPERACION_INDIVIDUAL, request);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/cxi/eliminar/{id}", method = RequestMethod.GET)
	 ...
	 * Paginado
	 */
	@RequestMapping(value = "/cxis/paginado/{page}/{size}/{operacion}", method = RequestMethod.GET)
	public String paginarEntidad(@PathVariable Integer page, @PathVariable Integer size, @PathVariable String operacion, Model model,  PageableSG pageable, HttpServletRequest request) {
		...
		try{
			...
			filtro = (TblContrato)request.getSession().getAttribute("sessionFiltroContratoCriterio");
			model.addAttribute("filtro", filtro);
